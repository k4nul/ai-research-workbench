...
export async function refreshProjectProgress(
  ...
  const result = await client.query<ProgressEvidence>(
    "SELECT " +
      ...
      "(p.qa_passed_at IS NOT NULL AND NOT EXISTS (SELECT 1 FROM qa_findings q WHERE q.project_id = p.id AND q.severity = 'BLOCKER' AND q.resolution_status = 'OPEN')) AS qa_ready, " +
